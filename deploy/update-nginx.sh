...
# The apex (latency.space) is Cloudflare-proxied, so without this nginx sees the
# CF edge IP for every apex request - real visitor IPs are lost from the logs
# and per-IP rate limiting keys on the shared CF address. Trust the CF ranges
# and take the real client IP from the CF-Connecting-IP header. Ranges are
# fetched fresh from Cloudflare so they stay current; grey-cloud subdomains lack
# the header and keep their real direct IP. Written to conf.d (http context).
blue "Configuring Cloudflare real-IP..."
REALIP_CONF="/etc/nginx/conf.d/cloudflare-realip.conf"
CF_RANGES="$( { curl -fsS --max-time 10 https://www.cloudflare.com/ips-v4; echo; curl -fsS --max-time 10 https://www.cloudflare.com/ips-v6; } 2>/dev/null | grep '/' )"
if [ "$(printf '%s\n' "$CF_RANGES" | grep -c '/')" -ge 5 ]; then
  {
    echo "# Generated by update-nginx.sh from Cloudflare's published IP ranges."
    printf '%s\n' "$CF_RANGES" | while read -r cidr; do
      [ -n "$cidr" ] && echo "set_real_ip_from $cidr;"
    done
    echo "real_ip_header CF-Connecting-IP;"
  } > "$REALIP_CONF"
  green "Wrote $REALIP_CONF ($(grep -c set_real_ip_from "$REALIP_CONF") ranges)"
else
  yellow "Could not fetch Cloudflare ranges; leaving real-IP config unchanged"
fi
