...
docker build -t latency-proxy-test-new -f Dockerfile.proxy .

echo "Checking if the build was successful..."
if docker image inspect latency-proxy-test-new &> /dev/null; then
...
docker run -d --name latency-proxy-test-container latency-proxy-test-new || {
