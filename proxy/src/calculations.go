...
func julianDateToTime(jd float64) time.Time {
	...
	dayFrac := jdFrac
	hours := math.Floor(dayFrac * 24)
	dayFrac -= hours / 24
	minutes := math.Floor(dayFrac * 1440)
	dayFrac -= minutes / 1440
	seconds := dayFrac * 86400
...
// Calculate distance between two specific objects
func calculateDistanceBetweenObjects(objects []CelestialObject, t time.Time) {
	fmt.Print("Enter first object name: ")
	var name1, name2 string
	fmt.Scanln(&name1)
	
	fmt.Print("Enter second object name: ")
	fmt.Scanln(&name2)
	
	// Find the objects
	obj1, found1 := findObjectByName(objects, strings.TrimSpace(name1))
	obj2, found2 := findObjectByName(objects, strings.TrimSpace(name2))
	...
	// Calculate distance
	distance := CalculateDistance(obj1, obj2, objects, t)
	
	// Check for occlusion
	occluded, occluderObj := IsOccluded(obj1, obj2, objects, t)
	
	// Display results
	fmt.Printf("\nDistance from %s to %s on %s:\n", 
		obj1.Name, obj2.Name, t.Format("2006-01-02"))
	fmt.Printf("%.0f km (%.2f million km)\n", distance, distance/1000000)
	
	if occluded {
		fmt.Printf("%s is currently occluded from %s's view by %s\n",
			obj2.Name, obj1.Name, occluderObj.Name)
	} else {
		fmt.Printf("%s is visible from %s\n", obj2.Name, obj1.Name)
	}
}

// Check for occultations
func checkOccultations(objects []CelestialObject, t time.Time) {
	fmt.Print("Enter observer object name: ")
	var observerName string
	fmt.Scanln(&observerName)
	
	// Find the observer object
	observer, found := findObjectByName(objects, strings.TrimSpace(observerName))
	if !found {
		fmt.Printf("Object '%s' not found\n", observerName)
		return
	}
	
	fmt.Printf("\nOccultations from %s on %s:\n\n", observer.Name, t.Format("2006-01-02"))
	
	// Store occultation results
	type OccultationEntry struct {
		Target    CelestialObject
		Distance  float64
		OccludedBy CelestialObject
	}
	
	occultations := make([]OccultationEntry, 0)
	
	// Check for occultations for all objects except the observer
	for _, target := range objects {
		if target.Name != observer.Name && target.Name != "" {
			occluded, occluderObj := IsOccluded(observer, target, objects, t)
			
			if occluded {
				distance := CalculateDistance(observer, target, objects, t)
				
				occultations = append(occultations, OccultationEntry{
					Target:    target,
					Distance:  distance,
					OccludedBy: occluderObj,
				})
	...
	// If no occultations found
	if len(occultations) == 0 {
		fmt.Println("No occultations found at this time.")
		return
	}
	
	// Sort by target name
	sort.Slice(occultations, func(i, j int) bool {
		return occultations[i].Target.Name < occultations[j].Target.Name
	})
	
	// Display results
	fmt.Printf("%-15s | %-10s | %-15s | %s\n", 
		"Target", "Type", "Distance", "Occluded By")
	fmt.Println("-----------------------------------------------------------")
	
	for _, entry := range occultations {
		fmt.Printf("%-15s | %-10s | %-15s | %s\n", 
			entry.Target.Name, 
			entry.Target.Type,
			FormatDistance(entry.Distance),
			entry.OccludedBy.Name)
	}
}

// List active transmitting spacecraft
func listActiveSpacecraft(objects []CelestialObject, t time.Time) {
	fmt.Println("\nActive Transmitting Spacecraft:")
	fmt.Println("===========================")
	
	fmt.Printf("%-15s | %-15s | %-15s | %-10s | %s\n", 
		"Name", "Distance", "Frequency", "Launch", "Status")
	fmt.Println("-------------------------------------------------------------------------")
	
	earth, _ := findObjectByName(objects, "Earth")
	
	count := 0
	for _, obj := range objects {
		if obj.Type == "spacecraft" && obj.TransmitterActive {
			// Calculate distance from Earth
			distance := CalculateDistance(earth, obj, objects, t)
			
			// Display info
			fmt.Printf("%-15s | %-15s | %-15.2f | %-10s | %s\n", 
				obj.Name, 
	...
	if count == 0 {
		fmt.Println("No active transmitting spacecraft found.")
	}
}